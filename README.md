...
`unitpostcom/skills` whenever `skills/**` changes on `main`. Auth is
`SKILLS_MIRROR_TOKEN` (same PAT as `email-mirror.yml`); it must list both
`unitpostcom/skills` and `unitpostcom/email`. See `infra/SECRETS.md` §2.7. The
library skill (`npx skills add unitpostcom/email`) lives in `packages/email`
and mirrors with that package, not this folder.
